...
- オーナーはXのPremium Plus会員であり、通常の280文字制限を前提に文案を短縮しない。文字数へ収めることより、読みやすさ、訴求力、オーナーらしい語りを優先する
- 記事URLとカバー画像を付ける
...
dev.toへ出るのは1 runで最大1本とする。滞留が溜まっても`workflow_dispatch`を連続実行して一度に消化しない。手動実行はワークフローの動作確認のように単発で必要な時だけ行い、その都度オーナーの指示を受ける。

出しすぎた記事は`workflow_dispatch`の`hide_slugs`にZenn slugをカンマ区切りで渡し、`--hide`でdev.toの下書きへ戻す。台帳entryに`hidden: true`が付き、記事URLとidは保持される。以降は毎日のrunが`--republish`で公開日の古い順に1件だけ再公開し、再公開した日は新規転載を行わない。`hidden`が全て消えると通常の新規転載へ戻る。

`hidden`の記事は読者から見えないため、そこへ向く内部リンクは書き換えず`pending`に残し、再公開後に`--fix-links`が解決する。`hidden`の記事自体もリンク更新の対象にしない。
