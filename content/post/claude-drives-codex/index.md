...
draft: false
description: "aitermで握った永続ターミナルの本当のうまみは、対話型コマンドがそのまま回せることだった。つまり親のClaudeが中でCodex CLIを起動して子分にできる。rpgdevの曲も絵も効果音も、並走させるとぶつかる創作作業を、親Claudeの管理下にCodexを置いて衝突なく丸投げした話。"
tags: ["Claude Code", "Codex", "aiterm-mcp", "サブエージェント", "個人開発"]
cover:
  image: "cover.png"
---

[aiterm-mcp]({{< relref "aiterm-release" >}}) を「AIに永続ターミナルを1本握らせてトークンを削る」道具として公開した。けど使ってるうちに、永続ターミナルの本当のうまみは別のところにあると気づいた。**対話型のコマンドが、そのまま回せる**ことだ。
...
![並走（横並び）は同じリポジトリを2AIが奪い合ってコミットが枝分かれする。親子（縦積み）は親ClaudeがaitermでCodexを握り、順番を直列に捌くから衝突しない。](fig1.png)
...
![『この敵の絵を描け』で返ってきたボススプライト。ひび割れた黒曜石の体に溶岩脈が走るMagma Golem。透過背景でそのままゲームに乗る。](boss.png)
