...
date: 2026-05-20
draft: false
description: "メインPCを寝かせたくて Raspberry Pi 5 で低消費電力の監視端末を自作。OLEDをエヴァ風モニタにして、4.3インチには動画を流し始めた話。"
tags: ["Claude Code", "Raspberry Pi", "サーバー", "MAX"]
cover:
  image: "cover.png"
---
...
![省エネのため、大きなメインPCの代わりに小さな低消費電力の端末へ見張り役を移す](eyecatch.png)

サーバー本体を MS-A2 に乗り換えた話は[前回の記事]({{< relref "/post/bc250-to-ms-a2" >}})で書いた。今回はその鯖を「見張る側」を独立させる話。
...
ここはもう驚きがない。Claude にサーバー周りを任せる話は[何度も書いてきた]({{< relref "/post/ai-server-management" >}})し、淡々と終わる。
...
![OLEDの3状態。正常は角ブラケットだけ、警告で上下にストライプ、異常で全周がストライプ枠に変わる](oled.png)
...
![Pi 5 監視機。左の OLED が「正常」、右の 4.3 インチは動画再生中](pi5.jpg)
