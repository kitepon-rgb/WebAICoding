...
draft: false
description: "動くものが勝手に進むのを眺めるのが好きだ。AIが働いてるのを見るのも好き。でも、ターミナルのスクロールをただ眺めるのにも限度がある。だから待ち時間を少しだけ面白くした。作ったのは rpgdev——Macの画面の隅で、AIがコードを書いてる間ずっと小さなRPGが勝手に進むやつ。ツールを使うと敵が出て、TODOがクエストになり、サブの作業役が精霊の仲間になる。一番苦労したのは仕組みじゃなく、ドット絵の方だった。完全に無意味なものを作った記録。"
tags: ["個人開発", "Claude Code", "Codex", "AIコーディング", "画像生成"]
cover:
  image: "cover.png"
---
...
![rpgdev のオーバーレイ実画面。左下で勇者が技を出し、中央に敵、右上にAIのTODOがクエストとして並ぶ](overlay.png)
...
![3つの舞台背景。左から草原（FIELD）・洞窟（DUNGEON）・城（CASTLE）。作業が進むほど右へ移る](stages.png)
...
![草原・洞窟・城に出る15体のモンスターと、勇者＋火（Ignis）・地（Terra）・風（Sylph）・水（Aqua）の精霊。全部自作のドット絵](roster.png)
