...
![夜のサーバーラックを、灯りをともしたAIが静かに見守るイメージ](eyecatch.png)

[前回の記事]({{< relref "/post/ai-server-management" >}})で、自宅サーバーの管理をAIに丸ごと任せた話を書いた。深夜にAIがパトロールして、昼間は監視スクリプトが異常を検知したらAIが出動する仕組みだ。
...
![同時SSH接続がMaxStartups上限10を超えて自分を弾く因果と、並列→直列・リトライ追加の2段階修正](fig1.png)
...
![深夜パトロールが拾った異常: Nextcloudログ21.3GB・SELinux拒否1241件/24h・スワップとfail2banの推移](fig2.png)
