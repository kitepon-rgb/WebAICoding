...
![PCで動いているClaude Codeのセッションを、スマホがそのまま覗き込む。`/rc` の体験のイメージ](eyecatch.png)
...
![`/rc` を打つとセッションにQRコードが出る。コードはローカルPCで走り、スマホはその「窓」になる](fig2.png)
...
| 機能 | 何ができる | 自分の状況 |
|---|---|---|
| **Skills** | カスタムスキルを定義して自動で呼び出させる | まだ試行錯誤中 |
| **Claude Code Channels** | Telegram / Discord からセッションを操作 | `/rc` の代替として気になっている |
| **`/simplify`** | PR前に3エージェントでコードレビュー | 次に試す |
| **`/batch`** | 大規模変更を並列処理してPRを自動作成 | 規模が大きくなったら使いたい |

---

![この記事で触った機能の早見表。立ち上げ／操作のしかた／調整・立て直しの3グループ](fig1.png)
...
実際にClaude Codeでアプリを作った話は、[OLTranslator]({{< relref "/post/oltranslator-app" >}})と[LiveTR]({{< relref "/post/livetr-app" >}})の記事で書いている。Claude Codeが「コードを書く」以上の価値を出した話は「[Claudeと論文から組み立てた話]({{< relref "/post/claude-research-implementation" >}})」で。
