...
![自分だけが使ってきた道具を、棚に並べて売り物にするイメージ](eyecatch.png)
...
![左がハードコード塊の自分専用Bot、右がSaaS化したAuctionBOT。中央は856行のserver.jsを8モジュールに分割](fig1.png)
...
Botの本番デプロイには、自宅サーバーを使っている。[以前の記事]({{< relref "/post/claude-code-deploy" >}})で書いた通り、AIにSSHでサーバーを直接触らせるスタイルだ。deploy.shを一発叩けば、ビルドからコンテナ更新まで全部終わる。
