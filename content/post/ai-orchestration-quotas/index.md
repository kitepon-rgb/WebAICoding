...
draft: false
description: "一番上のモデルに全部任せたら、枠が溶けてよくコケた。だから作業をF・A・Hで仕分けて、Claudeの枠を使わない外部のサブスクへ配り、決めたルールをhookで刺すことにした。"
tags: ["オーケストレーション", "Claude Code", "Codex", "Grok", "hook"]
cover:
  image: "cover.png"
---
...
{{< linkcard url="https://blog.kitepon.dev/post/codex-max-mode-quota-drain/" title="Codexの最上位モードに任せたら、5時間の枠を1時間で溶かした話" desc="いちばん上のモードを選んだら、子のAIも全部いちばん上で走っていた。枠が一気に減った理由と直し方。" site="Claude Code 始めました" >}}
...
{{< linkcard url="https://blog.kitepon.dev/post/aiterm-release/" title="aiterm-mcp を npm に公開した — AIに1本の永続ターミナルを握らせてトークンを削るMCPサーバ" desc="対話型のコマンドを永続ターミナルで握らせる仕組み。Codex や Grok もここから動かす。" site="Claude Code 始めました" >}}
...
![依頼が着手ゲートを通り、まとめ役が仕分けと配置を決める。手足の作業はClaudeの枠を使わない外部（Codex・Grok）へ先に出し、Oracleは相談だけの窓口として別枠でつながる。結果はまとめ役のゲートで検証してから保存と記録へ戻る](orchestration-map.png)
