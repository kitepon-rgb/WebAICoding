...
draft: false
description: "Claudeが時刻を推測で答える瞬間に出くわして、別Claudeを並走させる監査役を作った。最初の設計でデーモンが74個立ち、何度も作り直してv1.2.4まで来た記録。"
...
メインのClaudeとは別に、**ツールカタログを完全に把握した監査役のClaude (Haiku 4.5) をセッションごとに常駐させる**。メインのClaudeの発話予定と最終応答を並走で見て、ツールを呼び忘れてたら指摘する。

| 状況 | メインClaudeの応答 | 監査役の指摘 |
|---|---|---|
...
自分で踏んだ罠を、別の経路でまた踏む。これがあって [Caveat]({{< relref "caveat-release" >}}) の必要性を強く感じた。同じ罠を二度踏まない仕組みがないと、こうなる。
...
- **Stop hook の差し戻しは2連続表示になる**。メインのClaudeが応答を返した後にhookが走る仕様なので、補正応答を出すとユーザーは「最初の応答 + 補正応答」を続けて見ることになる。入力時 (UserPromptSubmit) で先回りできれば理想で、応答後は保険。
- **Haiku の timeout でユーザー入力がブロックされる**。fail-open (パスして通す) にするかは検討中。

## Throughline / Caveat との関係

Spotter は同じ作者が作った [Throughline]({{< relref "throughline-release" >}}) と [Caveat]({{< relref "caveat-release" >}}) と、**哲学を共有する別プロダクト**。
...
3つに共通するのは **「メインのClaude本体に頼らない仕組み」**。3つとも同居できる。
