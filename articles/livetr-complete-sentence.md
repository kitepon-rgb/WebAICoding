...
![暗い机の上、英語動画の字幕オーバーレイがリアルタイムで日本語に変わっていく雰囲気カット](https://blog.kitepon.dev/post/livetr-complete-sentence/eyecatch.png)
*暗い机の上、英語動画の字幕オーバーレイがリアルタイムで日本語に変わっていく雰囲気カット*
...
そこで LiveTR は、切れ端をそのまま訳に回さない。いったん抱えておいて、次の5秒で続きが来たらつなぎ、完全な一文にしてから訳す。完全な文だけを翻訳に渡す——これが翻訳精度のキモで、前に[LiveTR を紹介した記事](https://blog.kitepon.dev/post/livetr-app/)でも要だと書いた所だ。
...
![見切りカウンタが逆向き（上）だと前の切れ端が次の話者にくっつき、正しい向き（下）だと別々の発話として確定する](https://blog.kitepon.dev/post/livetr-complete-sentence/fig1.png)
*見切りカウンタが逆向き（上）だと前の切れ端が次の話者にくっつき、正しい向き（下）だと別々の発話として確定する*
...
- [LiveTR — 動画の英語音声をリアルタイムで日本語にするアプリ](https://blog.kitepon.dev/post/livetr-app/) — このアプリそのものの紹介。音声認識・翻訳・字幕・読み上げの全体像
